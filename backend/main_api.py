...
async def create_library(library_data: LibraryCreate, database: Session = Depends(get_db)) -> Library:
    ...
    db_library = Library(
        web_page=library_data.web_page,        telephone=library_data.telephone,        name=library_data.name,        address=library_data.address        )
...
async def bulk_create_library(items: list[LibraryCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_library = Library(
                web_page=item_data.web_page,                telephone=item_data.telephone,                name=item_data.name,                address=item_data.address            )
            database.add(db_library)
            database.flush()  # Get ID without committing
...
async def execute_library_cheapest_book_by(
    ...
    try:
        ...
        async def wrapper(_library_object):
            cheapest: Book = None
            price = 1000000000.0
            book_8758140672749 = (await get_books_of_library(_library_object.id, database))['books']
            for i in range(0, len(book_8758140672749)):
                book = book_8758140672749[i]
...
async def create_book(book_data: BookCreate, database: Session = Depends(get_db)) -> Book:
    ...
    db_book = Book(
        stock=book_data.stock,        genre=book_data.genre.value,        price=book_data.price,        pages=book_data.pages,        title=book_data.title,        release=book_data.release        )
...
async def bulk_create_book(items: list[BookCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_book = Book(
                stock=item_data.stock,                genre=item_data.genre.value,                price=item_data.price,                pages=item_data.pages,                title=item_data.title,                release=item_data.release            )
            database.add(db_book)
            database.flush()  # Get ID without committing
